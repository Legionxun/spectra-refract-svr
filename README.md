...
This version has added cloud synchronization functionality, where user data and various histories will be uploaded to specific GitHub repositories or downloaded locally, enabling users to share their data; It can also be uploaded to a custom repository.
...
​        该版本增加了云同步功能，用户数据和各种历史将会上传到特定的GitHub仓库里或下载到本地，实现了使用者们的数据共享；同时也可以上传到自定义的仓库中。
