...
class BatchResultsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        for i, result in enumerate(self.results):
            ...
            self.table.setItem(i, 1, QTableWidgetItem(f"{result['prediction']:.{decimal_places}f}"))
        ...
        if self.results:
            ...
            stats_label = QLabel(f"总计: {len(self.results)} 个文件  "
                                 f"平均折射率: {avg_prediction:.{decimal_places}f}  "
                    ...
                    df = pd.DataFrame([
                        {
                            ...
                            "折射率": round(result["prediction"], CONFIG.get("refractive_index_decimal_places", 4)),
                    ...
                    with open(file_path, 'w', newline='', encoding='utf-8-sig') as f:
                        ...
                        for result in self.results:
                            writer.writerow([
                                ...
                                round(result["prediction"], CONFIG.get("refractive_index_decimal_places", 4)),
                                result["confidence"],
                                self.model_name
                            ])
