...
class MenuBuilder:
    ...
    def build_menu(self):
        ...
        file_menu.addAction("导入数据1(原始数据) (O)", self.app.import_data_original)
        file_menu.addAction("导入数据2(绘图到80度) (Ctrl+Shift+O)", self.app.import_data_processed)
        file_menu.addSeparator()
        file_menu.addAction("保存当前系统输出内容 (S)", self.app.save_current_results)
        file_menu.addSeparator()
        ...
        data_menu.addAction("生成理论数据 (G)", self.app.generate_theoretical_data)
        data_menu.addAction("自定义生成理论数据(Ctrl+Shift+G)", self.app.custom_generate_theoretical_data)
        data_menu.addAction("停止生成(Z)", self.app.stop_generation)
        data_menu.addSeparator()
        data_menu.addAction("数据增强(Ctrl+Shift+U)", self.app.data_augmentation)
        self.menu_bar.addMenu(data_menu)

        # 模型操作菜单
        model_menu = QMenu("模型操作", self.menu_bar)
        model_menu.addAction("训练模型(T)", self.app.start_training)
        model_menu.addAction("停止训练(X)", self.app.stop_training)
        model_menu.addSeparator()
        model_menu.addAction("加载模型(L)", self.app.load_model)
        model_menu.addSeparator()
        model_menu.addAction("导出模型(E)", self.app.export_model)
        self.menu_bar.addMenu(model_menu)

        # 预测分析菜单
        predict_menu = QMenu("预测分析", self.menu_bar)
        predict_menu.addAction("预测折射率(P)", self.app.predict_refractive_index)
        predict_menu.addAction("批量预测(B)", self.app.batch_prediction)
        self.menu_bar.addMenu(predict_menu)

        # 查看分析菜单
        view_menu = QMenu("查看分析", self.menu_bar)
        view_menu.addAction("查看优化历史(O)", self.app.show_optimization_history)
        view_menu.addAction("查看可视化结果(V)", self.app.show_visualizations)
        view_menu.addSeparator()
        view_menu.addAction("模型比较(M)", self.app.compare_models)
        self.menu_bar.addMenu(view_menu)

        # 历史记录菜单
        history_menu = QMenu("历史记录", self.menu_bar)
        history_menu.addAction("查看预测历史(H)", self.app.show_prediction_history)
        history_menu.addAction("查看监控日志(N)", self.app.show_monitoring_logs)
        self.menu_bar.addMenu(history_menu)

        # 系统工具菜单
        tools_menu = QMenu("系统工具", self.menu_bar)
        tools_menu.addAction("刷新界面(F5)", self.app.refresh_page)
        tools_menu.addAction("清空图表(D)", self.app.init_result_frame)
        tools_menu.addAction("清空输出(Ctrl+Shift+D)", self.app.clear_output)
        tools_menu.addSeparator()
        tools_menu.addAction("系统监控(Y)", self.app.toggle_system_monitor)
        ...
        help_menu.addAction("自定义快捷键(Ctrl+Shift+K)", self.app.show_customize_shortcuts_dialog)
        ...
        self.menu_items = {
            ...
            "查看监控日志": (menus[4], menus[4].actions()[1]),

            # 系统工具菜单项
            "刷新界面": (menus[5], menus[5].actions()[0]),
            "清空图表": (menus[5], menus[5].actions()[1]),
            "清空输出": (menus[5], menus[5].actions()[2]),
            "系统监控": (menus[5], menus[5].actions()[4]),
        }
