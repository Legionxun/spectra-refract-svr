...
from PySide6.QtWidgets import (
    ...
    QHeaderView, QProgressBar, QPushButton, QTabWidget, QFileDialog,
    QMessageBox, QApplication, QWidget, QScrollArea, QToolBar
)
    ...
    def run_evaluation(self, force_recalculate=False):
        ...
        try:
            ...
            for img_path in self.template_images:
                ...
                try:
                    name_str = filename.split('_')[1].rsplit('.', 2)
                    actual_rn = int(name_str[0]) + int(name_str[1]) * round(math.pow(0.1, len(name_str[1])), len(name_str[1]))
            ...
            for i, model_info in enumerate(self.models_list):
                ...
                self.progress_updated.emit(progress, f"正在评估模型: {model_info['name']} ({i + 1}/{total_models})")
...
class ModelComparisonDialog(QDialog):
    ...
    def create_table_tab(self):
        ...
        self.table.setHorizontalHeaderLabels([
            "模型名称", "MAE", "RMSE", "MedAE", "SSE", "R²", "准确度(±0.001)(%)", "模型大小(MB)"
        ])

        # 设置列宽
        self.table.setColumnWidth(0, 150)  # 模型名称
        self.table.setColumnWidth(1, 100)  # MAE
        self.table.setColumnWidth(2, 100)  # RMSE
        self.table.setColumnWidth(3, 100)  # MedAE
        self.table.setColumnWidth(4, 100)  # SSE
        self.table.setColumnWidth(5, 100)  # R²
        self.table.setColumnWidth(6, 120)  # 准确度
        self.table.setColumnWidth(7, 100)  # 模型大小
    ...
    def populate_table(self):
        ...
        for i, data in enumerate(self.model_data):
            ...
            self.table.setItem(i, 1, QTableWidgetItem(f"{data['mae']:.4f}"))
            self.table.setItem(i, 2, QTableWidgetItem(f"{data['rmse']:.4f}"))
            self.table.setItem(i, 3, QTableWidgetItem(f"{data['median_ae']:.4f}"))
            self.table.setItem(i, 4, QTableWidgetItem(f"{data['sse']:.4f}"))
            self.table.setItem(i, 5, QTableWidgetItem(f"{data['r2']:.4f}"))
            self.table.setItem(i, 6, QTableWidgetItem(f"{data['accuracy']:.2f}"))
            self.table.setItem(i, 7, QTableWidgetItem(f"{data['model_size']:.2f}"))
    ...
    def generate_charts(self):
        ...
        try:
            ...
            fig, axes = plt.subplots(2, 4, figsize=(12, 6))  # 调整为更小的尺寸

            # MAE 比较图
            names = [d["name"] for d in self.model_data]
            mae_values = [d["mae"] for d in self.model_data]
            axes[0, 0].bar(names, mae_values, color='skyblue')
            axes[0, 0].set_title('平均绝对误差 (MAE)', fontsize=10)
            axes[0, 0].set_ylabel('MAE', fontsize=9)
            axes[0, 0].tick_params(axis='x', rotation=45, labelsize=8)

            # RMSE 比较图
            rmse_values = [d["rmse"] for d in self.model_data]
            axes[0, 1].bar(names, rmse_values, color='lightcoral')
            axes[0, 1].set_title('均方根误差 (RMSE)', fontsize=10)
            axes[0, 1].set_ylabel('RMSE', fontsize=9)
            axes[0, 1].tick_params(axis='x', rotation=45, labelsize=8)

            # SSE 比较图
            sse_values = [d["sse"] for d in self.model_data]
            axes[0, 2].bar(names, sse_values, color='lightgreen')
            axes[0, 2].set_title('残差平方和 (SSE)', fontsize=10)
            axes[0, 2].set_ylabel('SSE', fontsize=9)
            axes[0, 2].tick_params(axis='x', rotation=45, labelsize=8)

            # 中位数绝对误差比较图
            median_ae_values = [d["median_ae"] for d in self.model_data]
            axes[0, 3].bar(names, median_ae_values, color='gold')
            axes[0, 3].set_title('中位数绝对误差', fontsize=10)
            axes[0, 3].set_ylabel('MedAE', fontsize=9)
            axes[0, 3].tick_params(axis='x', rotation=45, labelsize=8)

            # R² 比较图
            r2_values = [d["r2"] for d in self.model_data]
            axes[1, 0].bar(names, r2_values, color='purple')
            axes[1, 0].set_title('决定系数 (R²)', fontsize=10)
            axes[1, 0].set_ylabel('R²', fontsize=9)
            axes[1, 0].set_ylim(0, 1)  # R²范围通常在0-1之间
            axes[1, 0].tick_params(axis='x', rotation=45, labelsize=8)

            # 模型大小比较图
            model_size_values = [d["model_size"] for d in self.model_data]
            axes[1, 1].bar(names, model_size_values, color='orange')
            axes[1, 1].set_title('模型大小', fontsize=10)
            axes[1, 1].set_ylabel('大小 (MB)', fontsize=9)
            axes[1, 1].tick_params(axis='x', rotation=45, labelsize=8)

            # 准确度比较图
            accuracy_values = [d["accuracy"] for d in self.model_data]
            axes[1, 2].bar(names, accuracy_values, color='violet')
            axes[1, 2].set_title('预测准确度 (%)', fontsize=10)
            axes[1, 2].set_ylabel('准确度(±0.001)(%)', fontsize=9)
        ...
        try:
            # 创建雷达图，使用合适的默认尺寸
            fig_radar = plt.figure(figsize=(6, 6))  # 调整为更小的尺寸
            ...
            metric_labels = ['MAE', 'RMSE', 'SSE', 'MedAE', 'R²', '准确度(±0.001)', '模型大小']
            ...
            for i, (name, color) in enumerate(zip(model_names, colors)):
                ...
                # 创建数据点
                data = [mae, rmse, sse, median_ae, r2, accuracy, model_size]
            ...
            try:
                ...
                with open(comparison_csv_path, 'w', newline='', encoding='utf-8') as f:
                    writer = csv.DictWriter(f, fieldnames=["模型名称", "MAE", "RMSE", "MedAE", "SSE",
                                                           "R²", "准确度(±0.001)(%)", "模型大小(MB)"])
                    writer.writeheader()
    ...
    def generate_score_chart(self):
        ...
        try:
            ...
            for data in self.model_data:
                ...
                score = (
                                norm_mae * 0.15 +  # MAE权重15%
                                norm_rmse * 0.15 +  # RMSE权重15%
                                norm_sse * 0.10 +  # SSE权重10%
                                norm_r2 * 0.25 +  # R²权重25%
                                norm_model_size * 0.10 +  # 模型大小权重10%
                                norm_accuracy * 0.25  # 准确度权重25%
                        ) * 100  # 转换为百分制
                scores.append(score)
                model_names.append(data["name"])

            # 创建综合评分图表，使用合适的默认尺寸
            fig, ax = plt.subplots(figsize=(8, 5))  # 调整为更小的尺寸
    ...
    def reset_chart(self, chart_type):
        """重置图表大小"""
        if chart_type == "plot" and hasattr(self, 'plot_canvas'):
            self.plot_canvas.figure.set_size_inches(12, 6)
            self.plot_canvas.draw()
        elif chart_type == "radar" and hasattr(self, 'radar_canvas'):
    ...
    def load_best_model(self):
        ...
        if best_model_name:
            try:
                ...
                if model_path:
                    # 加载模型
                    self.app.predictor = RefractiveIndexPredictor(model_path)
                    self.app.current_model_dir = model_path
                    self.app.status_var.setText("已加载")
                    # 注意：这里需要根据实际 PyQt 实现调整状态指示器的更新方式
