...
class PredictionHistoryManager:
    ...
    def save_prediction_to_history(self, filename, prediction, confidence, model_name):
        ...
        try:
            with open(user_history_path, 'a', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
        ...
        if os.path.exists(history_path):
            ...
            if content is None:
                try:
                    ...
                    used_encoding = 'utf-8-sig (with errors ignored)'
                except Exception as e:
    ...
    def _ensure_user_history_file(self):
        ...
        if not os.path.exists(user_history_path):
            ...
            if user_records:
                try:
                    with open(user_history_path, 'w', newline='', encoding='utf-8') as f:
                        writer = csv.writer(f)
                        writer.writerow(["时间戳", "文件名", "预测折射率", "置信度", "模型名称"])
