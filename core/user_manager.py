...
class UserManager:
    ...
    def __init__(self, db_path: str = None, showinfo: bool = True):
        """初始化用户管理器"""
        if db_path is None:
    ...
    def _init_encryption_key(self):
        """初始化加密密钥"""
        key_file = os.path.join(CONFIG["settings_dir"], "encryption.key")
    ...
    def save_remembered_user(self, username: str, password: str, remember: bool):
        ...
        try:
            ...
            if remember:
                ...
                token = self._hash_password(password)
                cursor.execute('''
                    INSERT OR REPLACE INTO remembered_users (username, password, token)
                    VALUES (?, ?, ?)
                ''', (username, encrypted_password, token))
            else:
                cursor.execute('''
                               DELETE
            ...
            # 更新内存中的记住用户列表
            self.remembered_users = self._load_remembered_users()
            logger.info(f"用户 {username} {('已' if remember else '已取消')}记住")
        except Exception as e:
            logger.error(f"用户记住失败: {str(e)}")
        ...
        try:
            ...
            for username, encrypted_password in results:
                try:
                    ...
                    decrypted_results.append((username, decrypted_password))
                except Exception as e:
                    logger.error(f"解密用户 {username} 的密码失败: {str(e)}")
                    # 如果解密失败，返回加密的密码
                    decrypted_results.append((username, encrypted_password))
    ...
    def _convert_avatar_path_to_relative(self, avatar_path: str) -> str:
        """将头像路径转换为相对路径"""
        if avatar_path and os.path.isabs(avatar_path):
            # 将绝对路径转换为相对于CONFIG["user_info"]的路径
            try:
                relative_path = os.path.relpath(avatar_path, CONFIG["user_info"])
    ...
    def _convert_avatar_path_to_absolute(self, avatar_path: str) -> str:
        """将头像路径转换为绝对路径"""
        if avatar_path and not os.path.isabs(avatar_path):
            # 将相对路径转换为绝对路径
            absolute_path = os.path.join(CONFIG["user_info"], avatar_path)
    ...
    def update_user_profile(self, username: str, nickname: str = None, gender: str = None,
        ...
        try:
            ...
            if avatar_path is not None:
                # 将头像路径转换为相对路径再保存
                relative_avatar_path = self._convert_avatar_path_to_relative(avatar_path)
                updates.append("avatar_path = ?")
                params.append(relative_avatar_path)
    ...
    def get_user_profile(self, username: str) -> Optional[dict]:
        ...
        try:
            ...
            if result:
                ...
                # 将相对路径转换为绝对路径
                if avatar_path:
                    absolute_avatar_path = self._convert_avatar_path_to_absolute(avatar_path)
                else:
                    absolute_avatar_path = avatar_path
                ...
                return {
                    ...
                    'gender': result[2],
                    'email': result[3],
                    'avatar_path': absolute_avatar_path
                }
            logger.error(f"获取用户个人资料成功！")
            return None
        except Exception as e:
