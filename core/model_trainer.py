# mtg_core/model_trainer.py
import os, joblib, logging, optuna, datetime, math, time, shutil, threading
import numpy as np
...
class ProgressSignal:
    """进度信号接口"""
    def __init__(self):
        self.progress_updated = ProgressCallback()  # 当前进度, 总进度, 描述
...
class ModelTrainer:
    ...
    def __init__(self, config=CONFIG, tuning_config=TUNING_CONFIG, app=None, training_worker=None):
        ...
        self.person_model_dir = None  # 用户自定义模型目录
    ...
    def _tune_with_optuna(self, X_train, y_train, initial_params=None, bayesian_history=None):
        ...
        def optuna_objective(trial, X, y):
            ...
            try:
                ...
                # 检查 study 是否被停止
                if hasattr(self.study, '_stop_flag') and self.study._stop_flag:
                    print(f"=== 检测到 study.stop() 被调用（Trial {trial.number}），提前退出 ===")
                    return np.inf
                ...
                print(f"Optuna objective function error: {str(e)}")
                return np.inf
    ...
    def run_training(self):
        ...
        if self.training_worker:
            def emit_training_message():
                if hasattr(self.training_worker, 'training_message') and self.training_worker.training_message:
                    if hasattr(self.training_worker.training_message, 'emit'):
                        self.training_worker.training_message.emit("分簇开始", "正在数据分簇...")
                    else:
                        self.training_worker.training_message("分簇开始", "正在数据分簇...")

            # 在新线程中调用以避免阻塞
            threading.Thread(target=emit_training_message, daemon=True).start()
        ...
        # 在主界面显示训练开始信息
        if self.training_worker:
            def emit_training_message2():
                if hasattr(self.training_worker, 'training_message') and self.training_worker.training_message:
                    if hasattr(self.training_worker.training_message, 'emit'):
                        self.training_worker.training_message.emit("训练开始", "训练中，请稍等...")
                    else:
                        self.training_worker.training_message("训练开始", "训练中，请稍等...")

            # 在新线程中调用以避免阻塞
            threading.Thread(target=emit_training_message2, daemon=True).start()
        ...
        if self.training_worker:
            def emit_training_message3():
                if hasattr(self.training_worker, 'training_message') and self.training_worker.training_message:
                    if hasattr(self.training_worker.training_message, 'emit'):
                        self.training_worker.training_message.emit("训练完成", f"模型已保存至 {self.model_dir} 目录")
                    else:
                        self.training_worker.training_message("训练完成", f"模型已保存至 {self.model_dir} 目录")

            # 在新线程中调用以避免阻塞
            threading.Thread(target=emit_training_message3, daemon=True).start()
