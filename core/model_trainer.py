...
import os, joblib, logging, optuna, datetime
import numpy as np
...
class ModelTrainer:
    def __init__(self, config=CONFIG, tuning_config=TUNING_CONFIG, app=None):
        ...
        try:
            for fname in os.listdir(self.config["data_path"]):
                ...
                try:
                    ...
                    label = int(parts[0]) + int(parts[1][:2].ljust(2, '0')) * 0.01
                    labels.append(label)
    ...
    def tune_hyperparameters(self, X_train, y_train):
        ...
        self.study.optimize(
            ...
            callbacks=[lambda study, trial: print(f"{datetime.datetime.now()} 试验{trial.number}完成，得到值为{trial.value:.8f}，参数为{trial.params}，当前最佳值: {study.best_value:.8f}")]
        )
    ...
    def run_training(self):
        ...
        # 在主界面显示训练开始信息
        if self.app:
            self.app.root.after(0, lambda: self.app.show_message("分簇开始", "正在数据分簇..."))
        ...
        if self.app:
            self.app.root.after(0, lambda: self.app.show_message("训练开始", "训练中，请稍等..."))
        ...
        # 在主界面显示训练完成信息和图片
        if self.app:
            self.app.root.after(0, lambda: self.app.show_training_results(
                self.model_dir, feature_plot_path, cluster_plot_path, result_plot_path))
