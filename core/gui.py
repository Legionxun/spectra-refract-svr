...
from PySide6.QtGui import QPixmap, QFont, QIcon, QColor, QPalette, QLinearGradient
...
from .utils import get_output_path, get_app_root
...
from .gui_components.cloud_sync_dialog import CloudSyncSettingsDialog
from .gui_components.status_bar import StatusBarManager
...
class ModelLoaderWorker(QThread):
    """模型加载工作线程"""
    finished = Signal(object)  # 成功时传递 predictor 实例
    ...
    def run(self):
        try:
            predictor = RefractiveIndexPredictor(self.model_dir, self.update_progress)
...
class RefractiveIndexApp(QMainWindow):
    ...
    training_message_signal = Signal(dict)
    comparison_message_signal = Signal(dict)
    def __init__(self):
        super().__init__()
        ...
        self.training_message_signal.connect(self.process_training_message)
        self.comparison_message_signal.connect(self.process_comparison_message)
        ...
        self.training_process = None  # 添加这一行来保存子进程引用
        self.training_thread = None  # 添加这一行来保存读取线程引用
        ...
        # 保存对状态栏组件的引用，以便其他方法可以访问
        self.status_label = self.status_bar_manager.status_label
        self.sync_progress_bar = self.status_bar_manager.sync_progress_bar
        self.sync_status_label = self.status_bar_manager.sync_status_label
        ...
        self.cloud_sync_manager.setup_auto_sync(self, start=True)
        ...
        try:
            ...
            def process_events_periodically():
                if self.generation_in_progress:
                    QApplication.processEvents()
                    QTimer.singleShot(100, process_events_periodically)  # 每100毫秒处理一次事件

            QTimer.singleShot(100, process_events_periodically)
    ...
    def start_training(self):
        ...
        try:
            ...
            if hasattr(self, 'current_user_role') and self.current_user_role:
                # 安全地将用户角色对象转换为可序列化的字典
                try:
                    # 尝试直接序列化用户角色对象
                    user_role_dict = {
                        "name": str(getattr(self.current_user_role, 'name', '')),
                        "value": str(getattr(self.current_user_role, 'value', '')),
                        "__dict__": {}
                    }
                    # 尝试获取__dict__属性中的内容
                    if hasattr(self.current_user_role, '__dict__'):
                        role_dict = getattr(self.current_user_role, '__dict__', {})
                        # 过滤掉不可序列化的对象
                        for key, value in role_dict.items():
                            try:
                                json.dumps(value)  # 测试是否可序列化
                                user_role_dict["__dict__"][key] = value
                            except (TypeError, ValueError):
                                # 如果不可序列化，转换为字符串
                                user_role_dict["__dict__"][key] = str(value)
                    user_info["user_role"] = user_role_dict
                except Exception as e:
                    self.logger.warning(f"序列化用户角色时出错，使用默认值: {str(e)}")
                    user_info["user_role"] = {
                        "name": "",
                        "value": "",
                        "__dict__": {}
            ...
            # 创建临时配置文件
            temp_config_file = os.path.join(CONFIG["temp_dir"], "training_config.json")
            ...
            # 获取训练脚本路径
            training_script = os.path.join(get_app_root(), "ModelTraining_Generator.py")

            # 启动训练子进程，传递配置文件路径作为参数
            self.training_process = subprocess.Popen(
                [sys.executable, training_script, temp_config_file],
            ...
            self.temp_file_path = os.path.join(os.path.dirname(training_script), "temp", "training_status.json")
            if os.path.exists(self.temp_file_path):
                os.remove(self.temp_file_path)

            # 在新线程中监控临时文件
            self.training_thread = threading.Thread(target=self.monitor_temp_file, daemon=True)
            self.training_thread.start()

        except Exception as e:
            print(f"启动训练子进程时出错: {str(e)}")
            self.logger.error(f"启动训练子进程时出错: {str(e)}")
            self.training_in_progress = False
            self.enable_all_buttons()

    def stop_training(self):
        """停止训练"""
        if self.training_in_progress and self.training_process:
            result = QMessageBox.question(self, "确认", "确定要停止训练吗？")
            if result == QMessageBox.Yes:
                ...
                try:
                    ...
                    self.logger.error(f"终止训练子进程时出错: {e}")

                print("已停止训练")
                QMessageBox.information(self, "提示", "已停止训练")
            else:
                return
        else:
            QMessageBox.warning(self, "警告", "当前没有正在进行的训练")
    ...
    def monitor_temp_file(self):
        """监控临时文件的变化"""
        try:
            while self.training_in_progress and self.training_process:
                # 检查临时文件是否存在且有更新
                if os.path.exists(self.temp_file_path):
                    try:
                        modified_time = os.path.getmtime(self.temp_file_path)
                        if modified_time > self.last_modified_time:
                            ...
                            # 在主线程中处理消息
                            self.training_message_signal.emit(data)
                    except (json.JSONDecodeError, FileNotFoundError):
                        pass
                    except Exception as e:
                        self.logger.error(f"读取临时文件时出错: {e}")

                # 短暂休眠以避免过度占用CPU
                time.sleep(0.1)

        except Exception as e:
            self.logger.error(f"监控临时文件时出错: {e}")
        finally:
            # 确保清理资源
            self.cleanup_training_monitor()

    def cleanup_training_monitor(self):
        """清理训练监控资源"""
        self.training_in_progress = False
    ...
    def process_training_message(self, data):
        """处理来自训练进程的消息"""
        try:
            ...
            if msg_type == "start":
                ...
                print(f"配置已更新: 优化方法={optimization_method}, 聚类方法={clustering_method}")
                ...
                self.logger.info(f"训练完成！模型已保存至 {model_dir} 目录")
                self.current_model_dir = model_dir
                self.training_in_progress = False
                self.stop_training_flag = False
                self.load_model_after_events(self.current_model_dir)
            elif msg_type == "error":
                print(f"训练出错: {message}")
                self.logger.error(f"训练出错: {message}")
                self.enable_all_buttons()
        except Exception as e:
            self.logger.error(f"处理训练消息时出错: {e}")
            self.enable_all_buttons()

    def load_model_after_events(self, model_dir):
        """在事件完成后加载模型"""
        try:
            ...
            self.status_var.setText("正在加载模型...")
            self.status_indicator.setStyleSheet("color: #ffc107;")

            # 创建并显示进度条窗口
            self.progress_dialog = ModelLoadingProgress(self)
            self.progress_dialog.show()

            # 启动模型加载线程
            self.model_loader_thread = ModelLoaderWorker(model_dir)
            ...
            self.model_loader_thread.progress.connect(self.on_model_loading_progress)
            self.model_loader_thread.start()

        except Exception as e:
            self.logger.error(f"启动模型加载时出错: {e}")
            print(f"启动模型加载时出错: {e}")
            # 显示错误消息
            QMessageBox.critical(self, "错误", f"启动模型加载失败: {str(e)}")
            self.enable_all_buttons()
    ...
    def on_model_loading_progress(self, value, text):
        """处理模型加载进度更新"""
        if hasattr(self, 'progress_dialog') and self.progress_dialog:
            self.progress_dialog.update_progress(value, text)

    def on_model_loaded(self, predictor):
        """模型加载完成的回调"""
        try:
            self.predictor = predictor
            self.current_model_dir = self.model_loader_thread.model_dir
            ...
            print(f"模型加载成功！目录: {self.current_model_dir}")
            self.logger.info(f"模型加载成功！目录: {self.current_model_dir}")

            # 关闭进度条窗口
            if hasattr(self, 'progress_dialog'):
                self.progress_dialog.close()

            QMessageBox.information(self, "成功", f"模型加载成功！\n目录: {os.path.basename(self.current_model_dir)}")
            self.show_visualizations()
        except Exception as e:
            self.logger.error(f"处理模型加载完成时出错: {e}")
        finally:
            # 确保按钮状态恢复正常
            self.enable_all_buttons()
    ...
    def on_model_load_error(self, error_msg):
        """模型加载失败的回调"""
        self.logger.error(f"加载模型失败: {error_msg}")
        ...
        QMessageBox.critical(self, "错误", f"无法加载模型: {error_msg}")
        self.enable_all_buttons()
    ...
    def predict_refractive_index(self):
        ...
        try:
            ...
            decimal_places = CONFIG.get("refractive_index_decimal_places", 4)
            if prediction is not None:
                confidence = max(0.85, min(0.99, 0.90 + (np.random.rand() * 0.08)))
                print(f"预测完成！折射率: {prediction:.{decimal_places}f}, 置信度: {confidence * 100:.1f}%")
                self.logger.info(f"预测完成！折射率: {prediction:.{decimal_places}f}, 置信度: {confidence * 100:.1f}%")
                ...
                # 显示预测结果
                decimal_places = CONFIG.get("refractive_index_decimal_places", 4)
                result_label = QLabel(f"折射率: {prediction:.{decimal_places}f}")
    ...
    def save_prediction_result(self, prediction):
        ...
        try:
            ...
            with open(txt_path, 'w', encoding='utf-8') as f:
                ...
                f.write(f"预测折射率: {prediction:.{max(decimal_places, 6)}f}\n")
    ...
    def compare_models(self):
        ...
        try:
            ...
            # 创建临时配置文件
            temp_file_path = os.path.join(CONFIG["temp_dir"], "model_comparison_config.json")
            ...
            # 获取模型比较脚本路径
            comparison_script = os.path.join(get_app_root(), "Model_Comparator.py")
            ...
            # 启动模型比较子进程，传递临时文件路径作为参数
            self.model_comparison_process = subprocess.Popen(
                [sys.executable, comparison_script, temp_file_path],
                creationflags=subprocess.CREATE_NO_WINDOW,
                encoding='utf-8',
                bufsize=1  # 行缓冲
            )
            ...
            # 初始化临时文件监控
            self.last_comparison_modified_time = 0

            # 在新线程中监控临时文件
            self.model_comparison_thread = threading.Thread(target=self.monitor_comparison_temp_file,
                                                           args=(temp_file_path,), daemon=True)
            self.model_comparison_thread.start()

        except Exception as e:
            print(f"启动模型比较子进程时出错: {str(e)}")
            self.logger.error(f"启动模型比较子进程时出错: {str(e)}")

    def monitor_comparison_temp_file(self, temp_file_path):
        """监控模型比较临时文件的变化"""
        try:
            while self.model_comparison_process and self.model_comparison_process.poll() is None:
                # 检查临时文件是否存在且有更新
                if os.path.exists(temp_file_path):
                    try:
                        modified_time = os.path.getmtime(temp_file_path)
                        if modified_time > self.last_comparison_modified_time:
                            self.last_comparison_modified_time = modified_time
                            ...
                            if isinstance(data_list, list):
                                for data in data_list:
                                    self.comparison_message_signal.emit(data)
                            else:
                                self.comparison_message_signal.emit(data_list)
                    except (json.JSONDecodeError, FileNotFoundError):
                        pass
                    except Exception as e:
                        self.logger.error(f"读取模型比较临时文件时出错: {e}")
            ...
            self.logger.error(f"监控模型比较临时文件时出错: {e}")
        finally:
            # 确保进程正确结束
            if hasattr(self, 'model_comparison_process') and self.model_comparison_process:
                self.model_comparison_process.wait()
            self.cleanup_comparison_monitor()

    def cleanup_comparison_monitor(self):
        """清理训练监控资源"""
        if hasattr(self, 'model_comparison_thread') and self.model_comparison_thread.is_alive():
            pass

    def process_comparison_message(self, data):
        """处理来自模型比较进程的消息"""
        try:
            msg_type = data.get("type", "")
            message = data.get("message", "")
            ...
            if msg_type == "start":
                print(message)
            elif msg_type == "model_performance":
                # 处理模型性能数据
                performance_data = data.get("data", {})
                model_name = performance_data.get("model_name", "未知模型")
                mae = performance_data.get("mae", 0)
                rmse = performance_data.get("rmse", 0)
                r2 = performance_data.get("r2", 0)
                ...
                print(f"模型比较出错: {message}")
                self.logger.error(f"模型比较出错: {message}")
            elif action == "load_best_model":
                model_path = data.get("model_path", "")
                if model_path and os.path.exists(model_path):
                    self.load_model_after_events(model_path)
            elif action == "close":
                print("模型比较应用已关闭")
        except Exception as e:
            self.logger.error(f"处理模型比较消息时出错: {e}")
    ...
    def open_cloud_sync_settings(self):
        """打开云同步设置对话框"""
        dialog = CloudSyncSettingsDialog(self)
        dialog.exec()

    def update_sync_progress(self, current, total):
        """更新同步进度"""
        self.status_bar_manager.update_sync_progress(current, total)

    def show_sync_progress(self, show=True):
        """显示或隐藏同步进度条"""
        self.status_bar_manager.show_sync_progress(show)
    ...
    def open_settings_dialog(self):
        ...
        if dialog.exec() == QDialog.Accepted:
            self.load_settings()
            QMessageBox.information(self, "提示", "设置已应用，请重新进行预测以查看效果")
        ...
        try:
            if os.path.exists(settings_file):
                with open(settings_file, 'r', encoding='utf-8') as f:
                    ...
                    # 更新折射率小数位数设置
                    CONFIG["refractive_index_decimal_places"] = settings.get("refractive_index_decimal_places", 4)
        except Exception as e:
    ...
    def show_about(self):
        ...
        # 版本信息
        version_label = QLabel("版本: 5.0.0 Stable")
        ...
        <li>NumPy & SciPy</li>
        <li>PySide6 Parent GUI & Tkinter Child GUI</li>
        </ul>
        """)
