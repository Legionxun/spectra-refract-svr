...
class RefractiveIndexPredictor:
    ...
    def __init__(self, model_dir, progress_callback=None):
        ...
        if progress_callback:
            progress_callback(5, "正在初始化模型加载...")
            time.sleep(0.1)
            progress_callback(10, "正在加载特征提取器...")
        ...
        if progress_callback:
            progress_callback(30, "特征提取器加载完成")
            time.sleep(0.1)

        # 加载其他组件
        model_path = os.path.join(model_dir, "models", CONFIG["save_model"])
        if progress_callback:
            progress_callback(35, "模型组件加载完成")

        self._create_main_module()
        if progress_callback:
            progress_callback(40, "主模块创建完成")

        data = joblib.load(model_path)
        if progress_callback:
            progress_callback(50, "加载模型数据完成")
        ...
        if progress_callback:
            progress_callback(70, "加载回归器完成")

        self.regressor = data["regressor"]
        self.best_params = data.get("best_params", {})  # 获取优化参数
        self.clustering_method = data.get("clustering_method", "kmeans")  # 获取聚类方法
        self.optimization_method = data.get("optimization_method", "optuna")
        self.training_time = data.get("training_time", 40.0 if self.clustering_method=="kmeans" else 200.0)
        self.browser_process = None
        self.browser_pids = set()  # 用于跟踪所有可能的浏览器进程

        if progress_callback:
            progress_callback(90, "最终初始化完成")
            time.sleep(0.1)
    ...
    def predict(self, img_path):
        ...
        try:
            ...
            self.logger.info(f"预测完成! 折射率: {prediction:.{decimal_places}f}")
            return round(prediction, max(decimal_places, 6))
    ...
    def close(self):
        ...
        # 清理特征提取器
        if hasattr(self, 'fe') and self.fe:
            try:
                del self.fe
            except Exception as e:
                self.logger.error(f"清理特征提取器时出错: {str(e)}")
        ...
        if hasattr(self, 'pipeline') and self.pipeline:
            try:
                ...
                self.logger.error(f"清理数据管道时出错: {str(e)}")
        ...
        if hasattr(self, 'regressor') and self.regressor:
            try:
                ...
                self.logger.error(f"清理回归器时出错: {str(e)}")

        self.logger.info("预测器已完全关闭")
        print("预测器已关闭，所有资源已释放")
